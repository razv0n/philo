...
/*   Updated: 2025/07/29 11:44:23 by mfahmi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    print_action(char *action, t_threads *philo)
{
        ...
        if (!philo->info->died)
            printf("%ld %d %s\n",time_fun() - philo->info->start_time, philo->philo_index, action);
        ...
        else if (!ft_strcmp(action, philo->info->str[EAT]))
        {
         ...
         else if (philo->info->nm_philo % 2 != 0 && !ft_strcmp(action, philo->info->str[THK]))
                    usleep(1000);
}
